...
module DataMapper
  ...
  def load_data
    ...
    mcollective_data = YAML.load_file(File.join(MCOLLECTIVE_DIR, 'config'))

    machine_config = mcollective_data.each_with_object({}).each do |mcollective_reply, machines|
      machines[mcollective_reply['sender']] = mcollective_reply['data']['interfaces']
    end
